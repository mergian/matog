...
namespace matog {
	namespace code {
//-------------------------------------------------------------------
Dyn::Dyn(const Field& root) : CodeTemplate(root, true), m_array(Static::getArray(m_root.getDynId())) {
	// init global
	init();
	...
	// init accessors
	Accessors acc(getDict(), &m_root, Params().setRoot(m_root.getDynName()));

	// generate
	generateFile("device_dyn", "Dyn", "cu");
}

//-------------------------------------------------------------------
void Dyn::initVariants(const Field& field) const {
	const Variant& variants = m_array.getVariant();

	if(field.isLayoutStruct()) {
		const Degree* degree = &Static::getDegree(field.getDynLayoutId());
	...
	if(field.isMultiDimensional()) {
		const Degree* degree = &Static::getDegree(field.getDynTranspositionId());
...
//-------------------------------------------------------------------
void Dyn::initTranspositions(Dict* dict, const Field& field, const Degree* degree) const {
	uint32_t value = 0;
...
//-------------------------------------------------------------------
void Dyn::initFlatItems(void) const {
			...
			if(it == reverse.rend()) {
				...
				if(reverse.empty()) {
					...
					for(const auto& pair : pairs) {
						...
						if(parent->isRoot()) {
							tmpl = !parent->isLayoutStruct() ? "device_dyn_ptr_R" : layout == matog::Layout::SOA ? "device_dyn_ptr_R_soa" : layout == matog::Layout::AOS ? "device_dyn_ptr_R_aos" : "device_dyn_ptr_R_aosoa";
						...
						if(tmpl) {
							...
							if(parent->isRoot()) {
								...
								for(const Field& f : parent->getChildren()) {
									if(&f == current)
										break;

									offset += f.getTreeSize();
								}

								set(incl, "OFFSET", offset);
								if(current->hasChildren()) {
									std::string name("__");
									name.append(current->getName());
									set(incl, "TYPE", name.c_str());
								} else {
	...
	std::function<void (const Field&)> rFields = [&](const Field& field) {
		...
		if(field.isDimensional()) {
			initCounts(field, "COUNTS", m_root.getExternalIndexType() == array::field::Type::U64);
...
//-------------------------------------------------------------------
void Dyn::initUnion(Dict* dict, const Field& current, const bool printParentCount) const {
	// is root?
	if(current.isRoot()) {
		...
		// data pointer
		set(inc(dict, "POINTER", m_root.isStruct() ? "data_pointer_struct" : "data_pointer_array"), "TYPE", m_root.getTypeName());
	} 
	// is child?
	else {
		const Field& parent = *current.getParent();
...
//-------------------------------------------------------------------
void Dyn::initRootFields(Dict* dict) const {
